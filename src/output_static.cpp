...
output_static::~output_static() { delete m_buffer; }

void output_static::put_byte(unsigned char value) {
  if (m_offset < m_capacity) {
    m_buffer[m_offset++] = value;
  } else {
    std::cerr << "Buffer overflow error: " << m_offset << ">" << m_capacity
              << '\n';
  }
}
...
unsigned char *output_static::getData() { return m_buffer; }

unsigned int output_static::getSize() { return m_offset; }
