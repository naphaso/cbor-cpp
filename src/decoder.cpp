...
decoder::decoder(input &in) {
    _in = &in;
    _state = decoder_state::STATE_TYPE;
}
...
void decoder::run() {
    unsigned int temp;
    while(1) {
        if(_state == decoder_state::STATE_TYPE) {
            if(_in->has_bytes(1)) {
                ...
                switch(majorType) {
                    case 0: // positive integer
                        if(minorType < 24) {
                            ...
                            _currentLength = 1;
                            _state = decoder_state::STATE_PINT;
                            ...
                            _state = decoder_state::STATE_ERROR;
                    ...
                    case 1: // negative integer
                        if(minorType < 24) {
                            ...
                            _currentLength = 1;
                            _state = decoder_state::STATE_NINT;
                        } else if(minorType == 25) { // 2 byte
                            _currentLength = 2;
                            _state = decoder_state::STATE_NINT;
                        } else if(minorType == 26) { // 4 byte
                    ...
                    case 2: // bytes
                        if(minorType < 24) {
                            _state = decoder_state::STATE_BYTES_DATA;
                            _currentLength = minorType;
                        } else if(minorType == 24) {
                            _state = decoder_state::STATE_BYTES_SIZE;
                            _currentLength = 1;
                        } else if(minorType == 25) { // 2 byte
                            _currentLength = 2;
                            _state = decoder_state::STATE_BYTES_SIZE;
                    ...
                    case 3: // string
                        if(minorType < 24) {
                            _state = decoder_state::STATE_STRING_DATA;
                            _currentLength = minorType;
                        } else if(minorType == 24) {
                            _state = decoder_state::STATE_STRING_SIZE;
                            _currentLength = 1;
                        } else if(minorType == 25) { // 2 byte
                            _currentLength = 2;
                            _state = decoder_state::STATE_STRING_SIZE;
                    ...
                    case 4: // array
                        if(minorType < 24) {
                            ...
                            _state = decoder_state::STATE_ARRAY;
                            _currentLength = 1;
                        } else if(minorType == 25) { // 2 byte
                            _currentLength = 2;
                            _state = decoder_state::STATE_ARRAY;
                    ...
                    case 5: // map
                        if(minorType < 24) {
                            ...
                            _state = decoder_state::STATE_MAP;
                            _currentLength = 1;
                        } else if(minorType == 25) { // 2 byte
                            _currentLength = 2;
                            _state = decoder_state::STATE_MAP;
                    ...
                    case 6: // tag
                        if(minorType < 24) {
                            ...
                            _state = decoder_state::STATE_TAG;
                            _currentLength = 1;
                        } else if(minorType == 25) { // 2 byte
                            _currentLength = 2;
                            _state = decoder_state::STATE_TAG;
                    ...
                    case 7: // special
                        if (minorType < 20) {
                            ...
                            _state = decoder_state::STATE_SPECIAL;
                            _currentLength = 1;
                        } else if(minorType == 25) { // 2 byte
                            _currentLength = 2;
                            _state = decoder_state::STATE_SPECIAL;
        ...
        } else if(_state == decoder_state::STATE_PINT) {
        ...
        } else if(_state == decoder_state::STATE_NINT) {
            if(_in->has_bytes(_currentLength)) {
                switch(_currentLength) {
                    case 1:
        ...
        } else if(_state == decoder_state::STATE_BYTES_SIZE) {
            if(_in->has_bytes(_currentLength)) {
                switch(_currentLength) {
                    case 1:
        ...
        } else if(_state == decoder_state::STATE_BYTES_DATA) {
            if(_in->has_bytes(_currentLength)) {
        ...
        } else if(_state == decoder_state::STATE_STRING_SIZE) {
            if(_in->has_bytes(_currentLength)) {
                switch(_currentLength) {
                    case 1:
                        _currentLength = _in->get_byte();
        ...
        } else if(_state == decoder_state::STATE_STRING_DATA) {
            if(_in->has_bytes(_currentLength)) {
                unsigned char *data = new unsigned char[_currentLength];
                _in->get_bytes(data, _currentLength);
                _state = decoder_state::STATE_TYPE;
        ...
        } else if(_state == decoder_state::STATE_ARRAY) {
            if(_in->has_bytes(_currentLength)) {
                switch(_currentLength) {
                    case 1:
        ...
        } else if(_state == decoder_state::STATE_MAP) {
            if(_in->has_bytes(_currentLength)) {
                switch(_currentLength) {
                    case 1:
        ...
        } else if(_state == decoder_state::STATE_TAG) {
            if(_in->has_bytes(_currentLength)) {
                switch(_currentLength) {
                    case 1:
        ...
        } else if(_state == decoder_state::STATE_SPECIAL) {
            if (_in->has_bytes(_currentLength)) {
                switch (_currentLength) {
                    case 1:
        ...
        } else if(_state == decoder_state::STATE_ERROR) {
            break;
        } else {
